...
async function processFirstAddressOrder(paymentInfo, invoiceId, params, client){
  ...
  const chat = { 
    ...
    messageArray: [
      { 
        from: 'dgoon', 
        message: `Hi. I will process your order soon. I am for 24 hrs, but during high volume times this might be delayed. You can message me here if you have any questions.
...
async function sanatizeFirstAddressOrderInfo(orderInfo){
  const addressInfoSchema = Joi.object({
    ...
    zipcode: Joi.alternatives().try(Joi.string().min(0).max(99999999999999), Joi.number().min(0).max(99999999999)).required(),
  ...
  const objectSchema = Joi.object({
    ...
    refundAddress: Joi.string().required().min(1).max(1100),
...
async function processFirstLockerOrder(paymentInfo, invoiceId, params, client){
  ...
  const chat = { 
    ...
    messageArray: [
      { 
        from: 'dgoon', 
        message: `Hi. I will try to process your order within 24 hours, but during times of high volume, this time might be delayed. You should check on your order
        every 24 hours. We can not issue refunds for items that were not picked up in time.
...
async function sanatizeFirstLockerOrderInfo(orderInfo){
  ...
  const objectSchema = Joi.object({
    ...
    itemList: itemListSchema,
    country: Joi.string().required().min(0).max(99),
    lockerZipcode: Joi.alternatives().try(Joi.string().min(0).max(9999999999999), Joi.number().min(0).max(999999999999999)).required(),
